# report_manager.py — MEMBER 2 OWNS THIS FILE
import sqlite3
import math
import os
...
def calculate_weight(hours_ago, upvotes=0, downvotes=0):
    # Factor 1: Recency exponential decay (half-life = 48 hours)
...
def update_all_weights(db_path=DB_PATH):
...
def get_weighted_incidents_near(lat, lng, radius_km=0.3, db_path=DB_PATH):
    if not os.path.exists(db_path):
        return []
    conn = get_db(db_path)
    d = radius_km / 111.0
...
def upvote_incident(incident_id, session_id, db_path=DB_PATH):
    conn = get_db(db_path)
    try:
...
def downvote_incident(incident_id, session_id, db_path=DB_PATH):
    conn = get_db(db_path)
    try:
        conn.execute('''INSERT INTO session_votes
        ...
        , (session_id, incident_id))
        conn.execute('''UPDATE incidents SET
        downvotes=downvotes+1 WHERE id=?''', (incident_id,))
        conn.commit()
        return True
    except Exception:
        return False
...
def get_recent_reports_in_city(city, limit=8, db_path=DB_PATH):
    if not os.path.exists(db_path):
        return []
    conn = get_db(db_path)
    try:
...
def get_city_stats(city, db_path=DB_PATH):
    ...
    try:
        ...
        ''', (city,)).fetchone()
        conn.close()
        return {'total': s[0] or 0, 'last_24h': s[1] or 0,
                'verified': s[2] or 0, 'avg_severity': round(s[3] or 0, 1)}
    except Exception:
        conn.close()
        return {'total': 0, 'last_24h': 0, 'verified': 0, 'avg_severity': 0}
