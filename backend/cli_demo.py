...
def main():
    ...
    while True:
        ...
        print("3. 📰  Fetch & Ingest Live News Incidents for City (AI News)")
        print("4. 🔥  View Recent Reports & Upvote Incidents")
        print("5. 🧠  Ask SafeWalk AI Assistant (RAG WHO/NCRB Knowledge)")
        print("6. 🌐  Change Selected City")
        print("7. ❌  Exit Demo")
        print("=" * 60)
        ...
        if choice == "1":
            ...
            print(f"\n📰 FETCHING LIVE NEWS & PUBLIC SAFETY ALERTS FOR {current_city.upper()}...")
            news_res = ingest_city_news_reports(city=current_city)

            print(f"\n✅ SUCCESS! Ingested {news_res['ingested_count']} news reports into Database.")
            print("\n📰 Fetched News Reports:")
            for item in news_res["raw_news_fetched"]:
                print(f"  • [{item.get('news_source', 'News')}] {item.get('location_description')}: {item.get('description')}")
            print("\n📊 Updated City Stats:", news_res["updated_city_stats"])
